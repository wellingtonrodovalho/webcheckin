        ...
        <div className="bg-white rounded-3xl shadow-xl p-6 sm:p-10">
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4">
              ...
              <div className="grid gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Escolha o Imóvel</label>
                  ...
                  <div className="p-3 bg-blue-50 rounded-lg mt-2">
                    <p className="text-[9px] font-bold text-blue-700 uppercase">Proprietário: {selectedProperty.ownerName}</p>
                    <p className="text-[9px] text-blue-500 uppercase">CPF: {selectedProperty.ownerCpf}</p>
                  </div>
                </div>
                ...
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  ...
                  <input type="number" name="totalValue" value={formData.reservation.totalValue || ''} onChange={handleReservationChange} placeholder="0,00" className="bg-transparent border-none p-0 focus:ring-0 font-black text-emerald-800 text-2xl w-full" />
                </div>
              </div>
              <button onClick={nextStep} disabled={!formData.reservation.startDate || formData.reservation.totalValue <= 0} className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl">PRÓXIMO PASSO</button>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4">
              ...
              <div className="grid gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FileUpload id="doc_main" label="Documento (Foto ou Anexo)" onFileSelect={handleUpload('main')} />
              ...
              <div className="flex gap-4">
                ...
                <button onClick={nextStep} disabled={!formData.mainGuest.documentFile} className="flex-[2] py-4 bg-blue-600 text-white font-black rounded-xl">PRÓXIMO: PET</button>
              </div>
            </div>
          )}
          ...
          {step === FormStep.PET_INFO && (
            <div className="space-y-8 animate-in slide-in-from-right-4">
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-400 text-xs font-bold uppercase">Pergunta obrigatória para o contrato</p>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <button 
                  onClick={() => handlePetToggle(false)}
                  className={`py-8 rounded-3xl border-4 transition-all flex flex-col items-center gap-3 ${!formData.pet.hasPet ? 'border-blue-600 bg-blue-50' : 'border-slate-100 bg-slate-50'}`}
                >
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${!formData.pet.hasPet ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
                    <i className="fas fa-times text-xl"></i>
                  </div>
                  <span className="font-black text-xs uppercase tracking-widest">NÃO LEVAREI</span>
                </button>

                <button 
                  onClick={() => handlePetToggle(true)}
                  className={`py-8 rounded-3xl border-4 transition-all flex flex-col items-center gap-3 ${formData.pet.hasPet ? 'border-orange-500 bg-orange-50' : 'border-slate-100 bg-slate-50'}`}
                >
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${formData.pet.hasPet ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-400'}`}>
                    <i className="fas fa-dog text-xl"></i>
                  </div>
                  <span className="font-black text-xs uppercase tracking-widest">SIM, LEVAREI</span>
                </button>
              </div>
              ...
              {formData.pet.hasPet && (
                <div className="space-y-4 p-6 bg-orange-50/50 border border-orange-100 rounded-3xl animate-in zoom-in-95">
                  ...
                  <FileUpload id="pet_vac" label="Carteira de Vacinação (Foto)" onFileSelect={handleUpload('pet')} />
                </div>
              )}
          ...
          {step === FormStep.SUCCESS && (
            <div className="text-center py-12 space-y-6">
              ...
              <h2 className="text-2xl font-black text-slate-800">ENVIADO COM SUCESSO!</h2>
              <p className="text-slate-500 text-sm">Suas fotos foram salvas e os links gerados para o contrato.</p>
              <button onClick={() => window.location.reload()} className="px-10 py-4 bg-slate-800 text-white font-black rounded-2xl">NOVO CADASTRO</button>
            </div>
          )}
        </div>
      </main>
